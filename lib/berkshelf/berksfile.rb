module Berkshelf
  class Berksfile
    ...
    def metadata(options = {})
      ...
      add_dependency(name, constraint, path: File.dirname(metadata_file))
    end
    ...
    #      solely on the locked dependencies.
    #    - If the SHAs don't match, then the Berksfile has diverged from the
    #      lockfile, which means some dependencies are outdated. For each unlocked
    #      dependency, see if there exists a locked version that still satisfies
    #      the version constraint in the Berksfile. If there exists such a
    #      dependency, remove it from the list of unlocked dependencies. If not, then
    #      either a version constraint has changed, or a new dependency has been
    #      added to the Berksfile. In the event that a locked_dependency exists,
    #      but it no longer satisfies the constraint, this method will raise
    #      a {Berkshelf::OutdatedDependency}, and inform the user to run
